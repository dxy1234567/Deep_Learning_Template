...
def split_dataset(dataset_dir, output_dir, train_ratio=0.8, val_ratio=0.1):
    ...
    depth_files = sorted(os.listdir(os.path.join(dataset_dir, "depth")))

    # 确保三者文件数量一致
    assert len(gt_files) == len(gray_files) == len(depth_files), "GT, Gray 和 Depth 文件数量不一致，请检查数据！"
    ...
    for subset, subset_indices in zip(subsets, [train_indices, val_indices, test_indices]):
        for folder in ["gt", "gray", "depth"]:
            copy_files(gt_files if folder == "gt" else gray_files if folder == "gray" else depth_files,
...
# 默认参数
dataset_dir = "dataset_total"  # 原始数据集路径
output_dir = "dataset_split"  # 输出数据集路径
train_ratio = 0.8  # 默认训练集比例
val_ratio = 0.1  # 默认验证集比例
